...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  if (!empty($_POST['pass'])) {
   $updateSQL = sprintf("UPDATE tb_admin SET Login=%s, Password=PASSWORD(%s), nama_admin=%s, gender_admin=%s, address_admin=%s, email_admin=%s, hp_admin=%s, key_admin=%s, active_admin=%s WHERE id_admin=%s",
                       ...
                       GetSQLValueString($koneksi, $_POST['pass'], "text"),
...
//mysqli_select_db($database_koneksi);
$query_UpdateAdmin = sprintf("SELECT * FROM tb_admin WHERE id_admin = %s", GetSQLValueString($koneksi, $colname_UpdateAdmin, "int"));
$UpdateAdmin = mysqli_query($koneksi, $query_UpdateAdmin) or die(errorQuery(mysqli_error()));
