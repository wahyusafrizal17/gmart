...
$rs_jumlah = mysqli_query($koneksi, $query_rs_jumlah) or die(errorQuery(mysqli_error()));
...
for ($a = 1; $a <= $totalRows_rs_jumlah; $a++) {
	if ((isset($_POST["MM_update".$a])) && ($_POST["MM_update".$a] == "form".$a)) {
	  $updateSQL = sprintf("UPDATE tb_admin SET photo_admin=%s WHERE id_admin=%s",
						   ...
						   GetSQLValueString($koneksi, $_POST['id_admin'.$a], "int"));
...
$rs_photos = mysqli_query($koneksi, $query_rs_photos) or die(errorQuery(mysqli_error()));
