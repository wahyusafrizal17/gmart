...
<?php if ($totalRows_Penjualan > 0) { ?>
  ...
  <table width="100%" class="tableku" border="2">
    ...
    do {
      ...
      //query hitung laba
      $query_laba = sprintf("SELECT faktur, tanggal, hargadasar, harga, diskon, qty, (hargadasar * qty) as hd, (harga * qty) as hj, SUM((((harga * qty) - (hargadasar * qty)))-diskon) as laba, ((harga * qty) - diskon) as sisadiskon FROM `transaksidetail` WHERE faktur = %s",  GetSQLValueString($koneksi, $row_Penjualan['kodefaktur'], "text"));
    ...
    <tr>
      <td colspan="5" align="right">TOTAL LABA</td>
      <td>
        <div align="right">Rp.
          <?= number_format($totallaba - $row_Total['jumlah']); ?></div>
      </td>
    </tr>
  </table>
  <p>
