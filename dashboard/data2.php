...
$query_rs_penjualan = sprintf("SELECT SUM(harga * qty) as Jumlah, SUM(qty) as Item,  count(faktur) as Transaksi FROM `transaksidetail` WHERE tanggal = CURDATE() AND periode = %s GROUP BY faktur",
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_item = sprintf("SELECT SUM(qty) as Item FROM `transaksidetail` WHERE tanggal = CURDATE() AND periode = %s",
	GetSQLValueString($ta, "text"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_return = sprintf("SELECT SUM(qtyreturn) as Jumlah FROM `returnproduk` WHERE tglreturn = CURDATE() AND periode = %s",
	GetSQLValueString($ta, "text"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_pendapatan = sprintf("SELECT SUM(harga * qty) as TotalHarga, tglfaktur FROM `transaksidetail` LEFT JOIN faktur ON faktur = kodefaktur WHERE statusfaktur = 'Y' AND faktur.periode = %s AND tglfaktur = CURDATE() GROUP BY tglfaktur",
	GetSQLValueString($ta, "text"));
