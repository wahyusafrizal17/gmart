...
// Build activity list (replaces missing history/activity1.php)
$currentPage = $_SERVER["PHP_SELF"];
	...
	if (isset($_GET['cari'])) {
		...
		$query_Produk = sprintf(
			"SELECT a.url, v.Nama, a.datetime FROM activity_update a 
			LEFT JOIN vw_login v ON a.oleh = v.ID 
			WHERE a.url LIKE %s OR v.Nama LIKE %s 
			ORDER BY a.id DESC",
			GetSQLValueString("%" . $colname . "%", "text"),
			GetSQLValueString("%" . $colname . "%", "text")
		);
	}else{
		$query_Produk = "SELECT a.url, v.Nama, a.datetime FROM activity_update a LEFT JOIN vw_login v ON a.oleh = v.ID ORDER BY a.id DESC";
	}	
