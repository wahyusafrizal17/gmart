...
// Fungsi untuk membuat join condition yang lebih robust
function getJoinCondition() {
    return "(b.nama = c.namaproduk OR b.kode = c.kodeproduk OR TRIM(b.nama) = TRIM(c.namaproduk))";
}
	...
	if ($jenisbayar == "0") {
		...
		$query_Penjualan = sprintf(
			...
			(SELECT SUM(b2.harga * b2.qty) FROM transaksidetail b2 
			 INNER JOIN produk c2 ON %s WHERE b2.faktur = a.kodefaktur AND c2.kategori = %s) AS totalbelanja,
			...
			AND EXISTS (SELECT 1 FROM transaksidetail b 
			           INNER JOIN produk c ON %s WHERE b.faktur = a.kodefaktur AND c.kategori = %s)
		...
		$query_Laba = sprintf(
			"SELECT SUM(laba) AS laba FROM (
				...
				(SELECT SUM(((b2.harga * b2.qty) - (b2.hargadasar * b2.qty)) - b2.diskon) FROM transaksidetail b2 
				 INNER JOIN produk c2 ON %s WHERE b2.faktur = a.kodefaktur AND c2.kategori = %s) AS laba
				FROM faktur a 
				WHERE (a.tglfaktur BETWEEN %s AND %s) AND a.statusfaktur = 'Y' AND a.addbyfaktur = %s AND a.periode = %s 
				AND EXISTS (SELECT 1 FROM transaksidetail b 
				           INNER JOIN produk c ON %s WHERE b.faktur = a.kodefaktur AND c.kategori = %s)
