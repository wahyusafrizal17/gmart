...
if(isset($_GET['kategori']) && ($_GET['kategori'] != 0) && isset($_GET['tgl1']) && isset($_GET['tgl2']) && (!isset($_GET['kasir']) || $_GET['kasir'] == 0) && (!isset($_GET['jenisbayar']) || $_GET['jenisbayar'] == "")) {
	...
	$query_Penjualan = sprintf(
		...
		(SELECT SUM(b2.harga * b2.qty) FROM transaksidetail b2 
		 INNER JOIN produk c2 ON b2.nama = c2.namaproduk 
		 WHERE b2.faktur = a.kodefaktur AND c2.kategori = %s) AS totalbelanja,
		...
		AND EXISTS (SELECT 1 FROM transaksidetail b 
		           ...
		           WHERE b.faktur = a.kodefaktur AND c.kategori = %s)
	...
	//total pendapatan - query yang lebih sederhana
	...
	$query_Laba = sprintf(
		"SELECT SUM(laba) AS laba FROM (
			SELECT DISTINCT a.kodefaktur,
			(SELECT SUM(((b2.harga * b2.qty) - (b2.hargadasar * b2.qty)) - b2.diskon) FROM transaksidetail b2 
			 INNER JOIN produk c2 ON b2.nama = c2.namaproduk 
			 WHERE b2.faktur = a.kodefaktur AND c2.kategori = %s) AS laba
			FROM faktur a 
			WHERE (a.tglfaktur BETWEEN %s AND %s) AND a.statusfaktur = 'Y' AND a.periode = %s 
			AND EXISTS (SELECT 1 FROM transaksidetail b 
			           INNER JOIN produk c ON b.nama = c.namaproduk 
...
try {
    ...
    if (!$rs_Penjualan) {
        throw new Exception("Error query Penjualan: " . mysqli_error($koneksi));
    }
    $row_Penjualan = mysqli_fetch_assoc($rs_Penjualan);

    $rs_total = mysqli_query($koneksi, $query_total);
    if (!$rs_total) {
        throw new Exception("Error query total: " . mysqli_error($koneksi));
    }
    $row_Total = mysqli_fetch_assoc($rs_total);

    $rs_pendapatan = mysqli_query($koneksi, $query_Pendapatan);
    if (!$rs_pendapatan) {
        throw new Exception("Error query pendapatan: " . mysqli_error($koneksi));
    }
    ...
    if (!$rs_laba) {
        throw new Exception("Error query laba: " . mysqli_error($koneksi));
    }
